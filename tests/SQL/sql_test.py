...
def make_all_graded_for_student(student_id: int = 1):
    submitted_assignments: Assignment = Assignment.filter(Assignment.student_id == student_id)
    count = 0
...
def test_get_assignments_in_graded_state_for_each_student():
    ...
    for student_id, graded in sql_result:
        if student_id == expected_result[0]:
            assert expected_result[0] == graded
...
def test_get_grade_A_assignments_for_teacher_with_max_grading():
    ...
    with open('tests/SQL/count_grade_A_assignments_by_teacher_with_max_grading.sql', encoding='utf8') as fo:
        sql = fo.read()
    # Create and grade 10 assignments for the default teacher (teacher_id=1)
    grade_a_count_1 = create_n_graded_assignments_for_teacher(10)
